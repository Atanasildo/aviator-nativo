...
// ── CRASH HANDLER ─────────────────────────────────────────────────────
async function registarCrash(valor) {
  ...
  // Guardar vela
  await guardarCrash(valor);
  
  // [NOVO] Gerar sinal global após cada crash
  gerarSinalGlobal();
}
...
// ── SINAIS GLOBAIS (NOVO) ────────────────────────────────────────────
let ultimoSinalGerado = 0;     // timestamp do último sinal global
let configRemotaCarregada = false;
let GROQ_KEY_REMOTA = '';
let DEEPSEEK_KEY_REMOTA = '';

// Carregar chaves da tabela config (mesmo que Android)
async function carregarConfigRemota() {
  try {
    const p = new urlMod.URL(CONFIG.SUPA_URL);
    const r = await httpReq({
      hostname: p.hostname,
      path: '/rest/v1/config?select=chave,valor',
      method: 'GET',
      headers: {
    ...
    if (r.status >= 200 && r.status < 300) {
      const configs = JSON.parse(r.body);
      configs.forEach(cfg => {
        if (cfg.chave === 'or_key1' && cfg.valor) GROQ_KEY_REMOTA = cfg.valor;
        if (cfg.chave === 'ds_key' && cfg.valor) DEEPSEEK_KEY_REMOTA = cfg.valor;
      });
      configRemotaCarregada = true;
      log(`✅ Config remota carregada (${GROQ_KEY_REMOTA ? 'OR' : ''}${DEEPSEEK_KEY_REMOTA ? '+DS' : ''})`);
    }
  } catch(e) {
    log(`⚠ Config remota falhou: ${e.message}`);
  }
}
...
async function buscarVelasSupabase(limite = 50) {
  try {
    const p = new urlMod.URL(CONFIG.SUPA_URL);
    const r = await httpReq({
      hostname: p.hostname,
      path: `/rest/v1/velas?select=coeficiente,timestamp&order=created_at.desc&limit=${limite}`,
      method: 'GET',
      headers: {
        'apikey': CONFIG.SUPA_KEY,
        'Authorization': `Bearer ${CONFIG.SUPA_KEY}`
    ...
    if (r.status >= 200 && r.status < 300) {
      const velas = JSON.parse(r.body);
      return velas.reverse();  // Ordem cronológica (antiga → nova)
    } else {
      log(`⚠ Erro ao buscar velas: ${r.status}`);
      return [];
    }
  } catch(e) {
    log(`✗ buscarVelasSupabase: ${e.message}`);
    return [];
  }
}

function construirPromptAnalise(velasArray) {
  if (!velasArray || velasArray.length < 10) {
    return null;
  }
  
  const n = velasArray.length;
  const media = velasArray.reduce((a,b) => a+b, 0) / n;
  const medianaSort = [...velasArray].sort((a,b) => a-b);
  const mediana = medianaSort[Math.floor(n/2)];
  const maxGeral = Math.max(...velasArray);
  const minGeral = Math.min(...velasArray);
  
  // Desvio padrão
  const variance = velasArray.reduce((acc, val) => acc + Math.pow(val - media, 2), 0) / n;
  const stdDev = Math.sqrt(variance);
  
  // Categorização
  const azuis = velasArray.filter(v => v < 2).length;
  const roxas = velasArray.filter(v => v >= 2 && v < 10).length;
  const rosas = velasArray.filter(v => v >= 10 && v < 50).length;
  const megas = velasArray.filter(v => v >= 50).length;
  
  const seqStr = velasArray.slice(-15).map(v => {
    if (v < 2) return '🔵';
    if (v < 10) return '⚪';
    if (v < 50) return '🩷';
    return '🟣';
  ...
  const prompt = `
Es un analizador especializado del juego Aviator (Spribe). Basándose en el histórico de velas, genera un sinal GLOBAL con ALTA ASSERTIVIDAD.

HISTÓRICO (${velasArray.length} rondas):
[${velasArray.map(v => v.toFixed(2)).join(', ')}]

SECUENCIA COLORES (últimas 15): ${seqStr}

ESTADÍSTICAS:
- Media: ${media.toFixed(2)}x | Mediana: ${mediana.toFixed(2)}x | Max: ${maxGeral.toFixed(2)}x
- Min: ${minGeral.toFixed(2)}x | StdDev: ${stdDev.toFixed(2)} | N: ${n}
- Azules: ${azuis} | Roxas: ${roxas} | Rosas: ${rosas} | Mega: ${megas}

Responde SOLO con JSON puro (sin explicaciones):
{
  "protecao": NUMERO,
  "alcance_min": NUMERO,
  "alcance_max": "NUMEROx",
  "tendencia": "SUBIDA|QUEDA|LATERAL",
  "confianca": PERCENTAGEM,
  "min_entrada": MINUTO
}

REGLAS CRITICAS:
- protecao: ~15% del alcance_max. NUNCA igual al alcance.
- alcance_min: realista basado en últimas rosas
- alcance_max: objetivo ambicioso, VARIAR siempre
- confianca: 30-95%, honesta y variada
- min_entrada: minuto entre 0 y 59`;
  
  return prompt;
}

async function chamarIaGlobal(prompt) {
  ...
  try {
    // 1. Tentar Groq com chave remota
    if (GROQ_KEY_REMOTA) {
      log('🧠 IA: Chamando Groq (remota)...');
      const r1 = await httpReq({
        hostname: 'api.groq.com',
        path: '/openai/v1/chat/completions',
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${GROQ_KEY_REMOTA}`,
      ...
      if (r1.status >= 200 && r1.status < 300) {
        ...
        return extrairJsonSinal(content);
      } else {
        log(`⚠ Groq: ${r1.status}`);
      }
    }
    
    // 2. Fallback: DeepSeek com chave remota
    if (DEEPSEEK_KEY_REMOTA) {
      log('⚠ Groq falhou, tentando DeepSeek (remota)...');
      ...
      const r2 = await httpReq({
        ...
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${DEEPSEEK_KEY_REMOTA}`,
          'Content-Type': 'application/json'
        }
      }, dsBody);
      ...
      if (r2.status >= 200 && r2.status < 300) {
        ...
        log(`⚠ DeepSeek: ${r2.status}`);
      }
    }
    
    log(`⚠ IA: Nenhuma chave remota configurada`);
    return null;
  } catch(e) {
    log(`✗ chamarIaGlobal: ${e.message}`);
    return null;
  }
}

function extrairJsonSinal(texto) {
  try {
    let clean = texto.replace(/```json/g, '').replace(/```/g, '').trim();
    const json = JSON.parse(clean);
    
    if (!json.protecao || !json.alcance_min || !json.alcance_max) {
      log(`⚠ JSON inválido: ${clean.substring(0,50)}`);
      return null;
    }
    
    return {
      protecao: parseFloat(json.protecao),
      alcance_min: parseInt(json.alcance_min),
      alcance_max: parseInt(json.alcance_max.toString().replace('x','')),
      tendencia: json.tendencia || 'LATERAL',
      confianca: Math.min(95, Math.max(30, parseInt(json.confianca) || 60)),
      min_entrada: parseInt(json.min_entrada) || -1
    };
  } catch(e) {
...
async function gerarSinalGlobal() {
  try {
    // Rate limiting: máximo 1 sinal por 2 minutos
    const agora = Date.now();
    if (agora - ultimoSinalGerado < 2 * 60 * 1000) {
      return;
    }
    
    const velas = await buscarVelasSupabase(50);
    if (velas.length < 10) {
      return;
    }
    
    const velasArray = velas.map(v => v.coeficiente);
    const prompt = construirPromptAnalise(velasArray);
    if (!prompt) return;
    
    const sinal = await chamarIaGlobal(prompt);
    if (!sinal) {
      return;
    }
    
    await guardarSinalGlobal(sinal, velas.length);
    ultimoSinalGerado = agora;
    
    log(`✅ Sinal global: prot=${sinal.protecao}x alc=${sinal.alcance_min}-${sinal.alcance_max}x conf=${sinal.confianca}%`);
  } catch(e) {
    log(`✗ gerarSinalGlobal: ${e.message}`);
  }
}

async function guardarSinalGlobal(sinal, numVelas) {
  const ts = new Date().toISOString();
  const body = JSON.stringify({
    protecao: sinal.protecao,
    alcance_min: sinal.alcance_min,
    alcance_max: sinal.alcance_max,
    tendencia: sinal.tendencia,
    confianca: sinal.confianca,
    min_entrada: sinal.min_entrada,
    velas_analisadas: numVelas,
    última_atualização: ts
  });
  ...
  try {
    ...
    if (r.status < 200 || r.status >= 300) {
      log(`⚠ Erro ao guardar sinal_global: ${r.status} ${r.body.slice(0,80)}`);
    }
  } catch(e) {
    log(`✗ guardarSinalGlobal: ${e.message}`);
  }
}
...
async function iniciar() {
  ...
  // Carregar chaves remotas da tabela config
